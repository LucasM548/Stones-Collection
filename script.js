...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function showToast(message, type = "info") { // type peut être 'info', 'success', 'error', 'warning'
  ...
  function createSVGCircles() {
    chakraCirclesData.forEach(chakra => {
        ...
        const chakraColor = getComputedStyle(document.documentElement).getPropertyValue(chakraColorVarName).trim();
  ...
  function showPanel(chakraId) {
    ...
    const activeChakraColor = getComputedStyle(document.documentElement).getPropertyValue(chakraColorVarName).trim();
  ...
  function hidePanel() {
    ...
    if (isEditing) { // Si on fermait pendant une édition, réinitialiser le formulaire
        resetForm();
    }
  }
  ...
  function generateStoneListItemHTML(stoneData) {
      ...
      return `
        ...
        <div class="stone-buttons">
          <button class="edit-stone-btn" data-stone-id="${stoneIdString}" aria-label="Modifier ${escapeHTML(stoneData.name)}" ${buttonStyle} title="Modifier">✎</button>
          <button class="delete-stone-btn" data-stone-id="${stoneIdString}" aria-label="Supprimer ${escapeHTML(stoneData.name)}" ${buttonStyle} title="Supprimer">×</button>
        </div>
      `;
  }
  ...
  function displayStonesForChakra(chakraId) {
    const stonesForCurrentChakra = (allStonesData[chakraId] || [])
        .filter(stone => stone && typeof stone === 'object' && stone.id !== undefined && stone.id !== null); // Filtre les données invalides
    ...
    if (stonesForCurrentChakra.length === 0) {
      ...
      const itemChakraColor = getComputedStyle(document.documentElement).getPropertyValue(chakraColorVarName).trim();
  ...
  async function handleFormSubmit(event) {
    ...
    if (savedStone && savedStone.id) {
      ...
      if (stoneIndex !== -1) { // Modification: remplace dans le tableau local
      ...
      } else { // Ajout: ajoute au tableau local
  ...
  function hideConfirmationModal() {
    ...
    }, { once: true }); // S'assure que l'écouteur ne s'exécute qu'une fois
  }
  ...
  function updateAdminUI() {
    ...
    adminLoginIcon.title = isAdmin ? "Mode Administrateur - Session Active" : "Mode Invité (cliquer pour connexion)";
  ...
  function checkAdminSessionValidity() {
    if (localStorage.getItem(ADMIN_SESSION_KEY) === "1") { // Si marqué comme admin
      if (getAdminSessionRemaining() > 0) { // Et si le temps n'est pas écoulé
        setAdminMode(true);
        return;
      }
    }
  ...
  function closeLoginModal() {
    ...
    adminLoginModal.classList.remove("visible"); // Lance l'animation de fermeture
     adminLoginModal.addEventListener('transitionend', () => { // Cache après l'animation
  ...
  function closeLogoutModal() {
    adminLogoutModal.classList.remove("visible"); // Lance l'animation de fermeture
     adminLogoutModal.addEventListener('transitionend', () => { // Cache après l'animation
  ...
  async function tryAdminLogin() {
    ...
    try {
      ...
      if (response.ok && result.success) { // Si succès
    ...
    } catch (error) { // Si erreur de communication
  ...
  confirmDeleteBtn.addEventListener('click', async () => {
    ...
    if (success) { // Si suppression réussie
  ...
  imageDropZone.addEventListener('click', (e) => {
      if (e.target !== removeImageBtn && !imageDropZone.classList.contains('has-image')) {
          imageInput.click();
      } else if (imageDropZone.classList.contains('has-image') && e.target !== removeImageBtn) {
  ...
  removeImageBtn.addEventListener('click', (e) => {
      e.preventDefault(); e.stopPropagation(); // Empêche le clic de déclencher l'ouverture du sélecteur
      resetImagePreview();
  });
  ...
  adminLoginIcon.addEventListener("click", () => {
    if (isAdmin) { // Si admin, ouvre la modale de déconnexion
    ...
    } else { // Si guest, ouvre la modale de connexion
